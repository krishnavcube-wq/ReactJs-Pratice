...
A modern, interactive React application for managing student records. This project showcases a beautiful UI with a hero carousel, glass‑morphism forms, and a responsive data table. Perfect for learning CRUD operations with a mock backend.

✨ Features
🏠 List Page – Table with alternating row colors, sticky header, and hover effects. Includes an auto‑playing hero carousel that pauses on hover.

➕ Add Student – Glass‑morphism form with a full‑page background image and validation.

✏️ Edit Student – Pre‑populated form to update existing records, with the same elegant design.

🗑️ Delete Student – Centered confirmation modal with a blur backdrop and smooth animations.
